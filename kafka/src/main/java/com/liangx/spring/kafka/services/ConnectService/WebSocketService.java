...
public class WebSocketService {
    ...
    public void onMessage(String message, Session session){
        ...
        switch (type){
            ...
            {
                userSessionManager.subscribeService_(session.getId(), "realMonitorService");
                break;
            }
            case MessageEntity.SUBSCRIBE_BACK_TRACKING_SERVICE :
            {
                userSessionManager.unsubscribeService_(session.getId(), "realMonitorService");
                userSessionManager.subscribeService_(session.getId(), "backTrackingService");
                break;
            }
            ...
            {
                userSessionManager.unsubscribeService_(session.getId(), "backTrackingService");
                userSessionManager.subscribeService_(session.getId(), "realMonitorService");
                break;
            }
            case MessageEntity.SUBSCRIBE_SCHEDULED_MONITOR_SERVICE : //订阅周监控缓服务
            {
                userSessionManager.subscribeService_(session.getId(), "scheduledMonitorService");
                break;
            }
        }
    ...
    private UserSessionManager getUserSessionManager(){
        return (UserSessionManager) IocUtil.getApplicationContext().getBean("userSessionManager");
    }
}
